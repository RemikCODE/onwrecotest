...
# Aim : to optimize ONHWRECO3 parameters emperically with new preprocessing strat
...
# Attempt 4: ONHWRECO3 with new preprocessed data. Does not converge
...
class ONHWRECO_test(HWRModel):
    def __init__(self, preload=False, gru=True):
        if gru:
            ...
            super().__init__()
            if preload:
                self.load_weights(PRETRAINED['ONHWRECO-LSTM'], full_path=True)
