...
def ndarray_map(f, a, as_list=False):
    ...
    try:
        ...
        # ragged input (e.g. sequences of different lengths) cannot be
        # stacked into a rectangular ndarray
...
def txt2label(txt, multiple=False):
    ...
    txt = list(map(lambda x: DATA.CHARS.index(x), txt))
...
def label2txt(labels, remove_dup=False, multiple=False):
    ...
    txt = list(map(lambda x: '' if x == -1 or x == DATA.BLANK_IDX else DATA.CHARS[int(x)], labels))
    txt = ''.join(txt)
    return txt
...
def label2mlf(labels, remove_dup=False, multiple=False):
    ...
    if remove_dup:
        labels = list(ch for ch, _ in itertools.groupby(labels))
    mlf = list(map(lambda x: '' if x == -1 or x == DATA.BLANK_IDX else DATA.CHARS_MLF[int(x)], labels))
...
def mlf2label(mlf, multiple=False):
    ...
    mlf_idx = list(map(lambda x: DATA.CHARS_MLF.index(x), mlf))
