...
class ONHWRECO(HWRModel):
    def __init__(self, preload=False, gru=False, decoder=None, gpu=False):
        if gru:
            self.rnn = GRU
            preload_key = "ONHWRECO-GRU" if preload else None
        else:
            self.rnn = LSTM
            preload_key = "ONHWRECO-LSTM" if preload else None
    ...
    def get_optimizer(self):
        return SGD(learning_rate=1e-4, momentum=0.9, nesterov=True, clipnorm=5)
        #return 'adam'
