...
class IAMSequence(Sequence):
    def __init__(self, split=SPLIT.ALL, batch_size=1, pred=False,
        ...
        if not self.npz:
            ...
            # Samples have different num of timesteps; store as object ndarray
            self.xs = np.array(
                [s.generate_features(preprocess_scheme) for s in tqdm(self.samples)],
                dtype=object)
            self.ys = np.array([s.ground_truth for s in self.samples], dtype=object)
            ...
            self.xs = np.array(self.xs, dtype=object)
            self.ys = np.array(self.ys, dtype=object)
